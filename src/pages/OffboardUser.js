...
window.App.OffboardUser = () => {
    ...
    return (
        <div className="bg-background-light dark:bg-background-dark text-text-primary-light dark:text-text-primary-dark min-h-screen flex flex-col">
            <StatusBar />
            <header className="bg-surface-light dark:bg-surface-dark px-4 py-3 border-b border-border-light flex items-center justify-between shrink-0 z-20 shadow-sm relative">
                <button onClick={() => navigate(-1)} className="p-1 -ml-1 text-text-secondary-light hover:text-primary transition rounded-full"><span className="material-icons-round text-2xl">close</span></button>
